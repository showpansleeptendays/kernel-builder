...
Random Forest (rf)
---------------------------------
This module implements bagging of decision trees.
...
class RfResult:
    ...
    def __repr__(self) -> str:
        return f"<RfResult result={self.result!r} meta={self.meta}>"

def core_algorithm(data: Any, **kwargs: Any) -> RfResult:
    """
    Core algorithm stub.
    Parameters
    ----------
    data : Any
        Input structure (bagging of decision trees).
    kwargs : Any
    ...
    return RfResult(result={"acc": acc, "trace": trace}, meta=meta)
...
def pretty_print(result: RfResult) -> str:
    """Create a human-readable summary."""
    acc = result.result.get("acc")
    pts = result.meta.get("trace_points")
    return f"Random Forest: acc={acc:.4f}, trace_points={pts}"
